...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await AuthService.register({ email, password, username: username || email.split('@')[0] });
      navigate('/login');
    } catch (err) {
      console.error(err);
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl p-8 border border-white/20">
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold text-white mb-2">Join MiniSocial</h1>
            <p className="text-white/80">Create your account to get started</p>
          </div>

          {error && (
            <div className="mb-6 p-4 bg-red-500/20 border border-red-300/30 rounded-xl backdrop-blur-sm animate-shake">
              <p className="text-white text-sm text-center">{String(error)}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-white/90">Username (optional)</label>
              <input
                type="text"
                value={username}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 px-4 bg-gradient-to-r from-green-500 to-teal-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
            >
              {loading ? (
          ...
          <div className="mt-6 text-center">
            <p className="text-white/80 text-sm">
              ...
              <Link
                to="/login"
                className="font-semibold text-white hover:text-green-200 transition-colors duration-300"
              >
                Sign In
              </Link>
            </p>
        ...
        <p className="text-center text-white/60 text-xs mt-4">
          © 2025 MiniSocial. All rights reserved.
        </p>
      </div>
